...
RUN rvm install ruby-4.0.6 -C --with-jemalloc \
  && rvm use --default ruby-4.0.6 \
  && rvm cleanup all
...
ENV PATH=${PATH}:/usr/local/rvm/rubies/ruby-4.0.6/bin
ENV PATH=${PATH}:/usr/local/rvm/gems/ruby-4.0.6/bin
ENV GEM_HOME /usr/local/rvm/gems/ruby-4.0.6
ENV GEM_PATH /usr/local/rvm/gems/ruby-4.0.6
