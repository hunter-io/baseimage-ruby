...
RUN rvm install ruby-3.4.5 -C --with-jemalloc \
  && rvm use --default ruby-3.4.5 \
  && rvm cleanup all
...
ENV PATH=${PATH}:/usr/local/rvm/rubies/ruby-3.4.5/bin
ENV PATH=${PATH}:/usr/local/rvm/gems/ruby-3.4.5/bin
ENV GEM_HOME /usr/local/rvm/gems/ruby-3.4.5
ENV GEM_PATH /usr/local/rvm/gems/ruby-3.4.5
