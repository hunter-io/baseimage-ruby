...
RUN rvm install ruby-3.2.0 -C --with-jemalloc \
  && rvm use --default ruby-3.2.0

# Set the required environment variables
ENV RACK_ENV production
ENV RAILS_ENV production
ENV NODE_ENV production
ENV PATH=${PATH}:/usr/local/rvm/rubies/ruby-3.2.0/bin
ENV PATH=${PATH}:/usr/local/rvm/gems/ruby-3.2.0/bin
ENV GEM_HOME /usr/local/rvm/gems/ruby-3.2.0
ENV GEM_PATH /usr/local/rvm/gems/ruby-3.2.0
