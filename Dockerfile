...
RUN rvm install ruby-4.0.0 -C --with-jemalloc \
  && rvm use --default ruby-4.0.0 \
  && rvm cleanup all
...
ENV PATH=${PATH}:/usr/local/rvm/rubies/ruby-4.0.0/bin
ENV PATH=${PATH}:/usr/local/rvm/gems/ruby-4.0.0/bin
ENV GEM_HOME /usr/local/rvm/gems/ruby-4.0.0
ENV GEM_PATH /usr/local/rvm/gems/ruby-4.0.0
