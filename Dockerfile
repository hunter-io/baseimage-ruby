...
RUN rvm install ruby-3.3.0 -C --with-jemalloc \
  && rvm use --default ruby-3.3.0 \
  && rvm cleanup all
...
ENV PATH=${PATH}:/usr/local/rvm/rubies/ruby-3.3.0/bin
ENV PATH=${PATH}:/usr/local/rvm/gems/ruby-3.3.0/bin
ENV GEM_HOME /usr/local/rvm/gems/ruby-3.3.0
ENV GEM_PATH /usr/local/rvm/gems/ruby-3.3.0
