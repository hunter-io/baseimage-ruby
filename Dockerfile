...
RUN rvm install ruby-4.0.3 -C --with-jemalloc \
  && rvm use --default ruby-4.0.3 \
  && rvm cleanup all
...
ENV PATH=${PATH}:/usr/local/rvm/rubies/ruby-4.0.3/bin
ENV PATH=${PATH}:/usr/local/rvm/gems/ruby-4.0.3/bin
ENV GEM_HOME /usr/local/rvm/gems/ruby-4.0.3
ENV GEM_PATH /usr/local/rvm/gems/ruby-4.0.3
